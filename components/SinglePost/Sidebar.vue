<template>
  <div class="col-lg-4">
    <div :class="`side-blog style-5 p${leftSidebar ? 'e' : 's'}-lg-5 mt-5 mt-lg-0`">
      ...
      <div class="side-recent-post mb-50">
        ...
        <NuxtLink
          ...
          v-for="(post, index) in data.recentPosts" 
          :key="index" 
          :class="`post-card ${index !== data.recentPosts.length - 1 ? 'pb-3 mb-3 border-bottom brd-gray' : ''}`"
        >
          <div class="img me-3">
      ...
      <div class="side-categories mb-50">
        ...
        <a href="#" class="cat-item" v-for="(category, index) in data.categories" :key="index">
      ...
      <div class="side-insta mb-50">
        ...
        <div class="d-flex justify-content-between flex-wrap">
          <a
            ...
            v-for="(image, index) in data.instagram" :key="index"  
      ...
      <div class="side-tags">
        ...
        <div class="content">
          <a href="#" v-for="(tag, index) in data.tags" :key="index" class="me-1">{{ tag }}</a>
        </div>
      </div>
    </div>
  </div>
...
<script>
export default {
  props: ['rtl', 'styleType', 'data', 'leftSidebar'],
  data() {
    return {
      index: null
    }
  ...
  computed: {
    gallery() {
      return this.data.instagram.map(slide => ({ url: slide }))
    }
  },
  methods: {
    showGallery(e, i) {
